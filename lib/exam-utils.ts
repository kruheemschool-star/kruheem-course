...
export const extractAnswerFromExplanation = (explanation: string): number | null => {
    ...
    //
    // กันไว้ด้วย (?![ก-๙]) คือ "ตัวอักษรที่จับได้ต้องไม่ใช่ต้นคำไทยคำอื่น"
    // เดิมกันด้วยการตัด ข ออกจากชุดตัวอักษร เพราะกลัวไปจับ ข ใน "ข้อ"
    // ผลคือ **อ่านเฉลยที่ตอบ ข ไม่ออกเลยทั้งคลัง** แล้วตกไปใช้ correctIndex ที่เก็บไว้แทน
    // และยังจับ "คำตอบคือ" เป็น ค ผิดๆ อีก (สระอือ ไม่ใช่ ้ จึงหลุดด่านเดิม)
    // รุ่นนี้ใส่ ข กลับเข้ามาแล้วกันด้วยตัวถัดไปแทน วัดกับคลังจริง 18,648 ข้อ:
    // อ่านไม่ออก 186 → 97 ข้อ และ **อ่านออกแต่ได้คำตอบผิด 24 → 0 ข้อ**
    const thaiMap: Record<string, number> = { 'ก': 0, 'ข': 1, 'ค': 2, 'ง': 3 };
    const thaiLetterPatterns = [
        /คำตอบ\s*:?\s*ข้อ\s*([กขคง])(?![ก-๙])/,
        /เฉลย\s*:?\s*ข้อ\s*([กขคง])(?![ก-๙])/,
        /คำตอบ\s*:?\s*([กขคง])(?![ก-๙])/,
        /เฉลย\s*:?\s*([กขคง])(?![ก-๙])/,
    ];
