...
/**
 * The enrollment form (course pick + payer info + slip upload + submit).
 * Assumes the visitor is ALREADY authenticated — the parent page gates on `user`
 * (logged-in users see this directly; logged-out users pass through the auth gate
 * first). On success it shows an in-page "thank you / wait for approval" screen.
 */
export default function EnrollmentForm() {
  const { user } = useUserAuth();
  // Firestore course docs are loosely shaped (same as the rest of the app).
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  ...
  useEffect(() => {
    const fetchCourses = async () => {
      try {
        ...
        const allCategories = Array.from(new Set(courseData.map((c) => (c as { category?: string }).category || "อื่นๆ")));
    ...
    try {
      ...
      if (originalSize > COMPRESSION_THRESHOLD) {
        ...
        try {
          ...
          console.warn('Compression attempt 1 failed:', workerErr instanceof Error ? workerErr.message : workerErr);
          try {
            ...
            console.warn('Compression attempt 2 failed:', fallbackErr instanceof Error ? fallbackErr.message : fallbackErr);
            compressedFile = file;
          }
        }
      }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Success → show the in-page thank-you / wait-for-approval screen.
      if (slipPreview) URL.revokeObjectURL(slipPreview);
      setSubmitted(true);
      ...
      } else if (e?.code === 'storage/retry-limit-exceeded' || e?.code === 'storage/server-file-wrong-size') {
  ...
  const filteredCourses = courses
    .filter((c: any) => !c.salesPage?.previewOnly) // "ปิดการขายชั่วคราว" — not yet released
  ...
  // ✅ Success screen — "we got it, wait for the teacher to approve"
  if (submitted) {
    return (
      <div className="relative z-10 bg-white/60 dark:bg-slate-900/80 backdrop-blur-3xl border border-white/60 dark:border-slate-700/50 shadow-2xl rounded-[3rem] p-8 sm:p-12 w-full max-w-2xl text-center text-slate-700 dark:text-slate-200 animate-in fade-in zoom-in-95 duration-500">
        <div className="inline-flex items-center justify-center w-20 h-20 rounded-full bg-emerald-100 dark:bg-emerald-900/40 text-5xl mb-5 shadow-inner">🎉</div>
        <h1 className="text-3xl font-black text-emerald-600 dark:text-emerald-400 mb-3">ได้รับข้อมูลเรียบร้อยแล้ว!</h1>
        <p className="text-slate-600 dark:text-slate-300 font-medium leading-relaxed mb-2">
          ครูได้รับการแจ้งโอนของคุณแล้ว 🙏<br />
          กำลังตรวจสอบสลิปและจะเปิดสิทธิ์เข้าเรียนให้โดยเร็วที่สุด
        </p>
        <p className="text-sm text-slate-400 dark:text-slate-500 mb-8">
          เมื่อครูอนุมัติแล้ว เข้าเรียนได้ทันที — เข้ามาเช็คสถานะได้ที่หน้า “คอร์สของฉัน”
        </p>
        <div className="flex flex-col gap-3">
          <Link
            href="/my-courses"
            className="w-full py-4 bg-gradient-to-r from-teal-500 to-emerald-600 hover:from-teal-400 hover:to-emerald-500 text-white font-bold text-lg rounded-2xl shadow-lg shadow-teal-500/30 transition-all hover:scale-[1.02] active:scale-95"
          >
            ไปหน้าคอร์สของฉัน
          </Link>
          <Link
            href="/"
            className="text-center text-slate-400 dark:text-slate-500 font-bold hover:text-teal-500 dark:hover:text-teal-400 transition text-sm"
          >
            กลับหน้าหลัก
          </Link>
        </div>
