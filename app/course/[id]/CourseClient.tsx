    ...
    useEffect(() => {
        ...
        const fetchData = async () => {
            try {
                ...
                try {
                    // Public student counter maintained by admin approve/delete actions —
                    // the same number the /exam page shows. Replaces a whole-collection
                    // COUNT aggregation that billed reads on every sales-page view.
                    const count = await getCachedData("public-stats-enrollments", async () => {
                        const statSnap = await getDoc(doc(db, "public_stats", "enrollments"));
                        return statSnap.exists() ? (statSnap.data().count as number | undefined) : undefined;
                    });
                    if (typeof count === "number") setTotalStudents(count);
