                                            ...
                                            {flashcardData.length > 0 && (
                                                <div className="bg-white p-4 rounded-2xl border-2 border-slate-100 shadow-sm max-h-[500px] overflow-y-auto">
                                                    <div className="flex items-center justify-between sticky top-0 bg-white pb-2 border-b z-10">
                                                        <h4 className="font-bold text-slate-700">📋 ตัวอย่างข้อมูล ({flashcardData.length} ใบ)</h4>
                                                        <div className="flex items-center gap-2">
                                                            {selectedCards.length > 0 && (
                                                                <button
                                                                    type="button"
                                                                    onClick={() => {
                                                                        const remaining = flashcardData.filter((_, idx) => !selectedCards.includes(idx));
                                                                        syncFlashcardEditor(remaining);
                                                                        setSelectedCards([]);
                                                                        showToast(`🗑️ ลบ ${selectedCards.length} การ์ดแล้ว`);
                                                                    }}
                                                                    className="text-[10px] bg-red-50 text-red-600 px-3 py-1.5 rounded-lg border border-red-200 hover:bg-red-100 transition font-bold flex items-center gap-1"
                                                                >
                                                                    <Trash2 className="w-3 h-3" />
                                                                    ลบที่เลือก ({selectedCards.length})
                                                                </button>
                                                            )}
                                                            <label className="text-[10px] text-slate-400 flex items-center gap-1 cursor-pointer select-none">
                                                                <input
                                                                    type="checkbox"
                                                                    className="rounded border-slate-300"
                                                                    checked={selectedCards.length === flashcardData.length && flashcardData.length > 0}
                                                                    onChange={(e) => {
                                                                        setSelectedCards(e.target.checked ? flashcardData.map((_, i) => i) : []);
                                                                    }}
                                                                />
                                                                เลือกทั้งหมด
                                                            </label>
                                                    ...
                                                    <table className="w-full text-sm text-left mt-2">
                                                        <thead className="text-xs text-slate-400 uppercase bg-slate-50 sticky top-10">
                                                            <tr>
                                                                <th className="px-2 py-2 w-8"></th>
                                                                <th className="px-2 py-2 w-10">ID</th>
                                                                <th className="px-2 py-2 w-28">Topic</th>
                                                        ...
                                                        <tbody>
                                                            {flashcardData.map((card, idx) => (
                                                                <tr key={idx} className={`border-b border-slate-50 align-top transition ${selectedCards.includes(idx) ? 'bg-red-50/50' : 'hover:bg-slate-50/50'} ${editingCardIdx === idx ? 'bg-yellow-50/50' : ''}`}>
                                                                    ...
                                                                    <td className="px-2 py-2 text-slate-400 font-mono text-xs">{card.id}</td>
                                                                    <td className="px-2 py-2">
                                                                        {editingCardIdx === idx ? (
                                                                            <input type="text" value={card.topic || ""} onChange={(e) => { const updated = [...flashcardData]; updated[idx] = { ...updated[idx], topic: e.target.value }; syncFlashcardEditor(updated); }} className="w-full px-2 py-1 text-xs border border-yellow-300 rounded-lg bg-yellow-50 outline-none focus:border-yellow-500" />
                                                                        ) : (
                                                                            <span className="text-slate-600 font-bold text-xs">{card.topic}</span>
                                                                        )}
                                                                    </td>
                                                                    <td className="px-2 py-2">
                                                                        {editingCardIdx === idx ? (
                                                                            <textarea value={card.question || card.front || ""} onChange={(e) => { const updated = [...flashcardData]; updated[idx] = { ...updated[idx], question: e.target.value }; syncFlashcardEditor(updated); }} className="w-full px-2 py-1 text-xs border border-yellow-300 rounded-lg bg-yellow-50 outline-none focus:border-yellow-500 min-h-[60px] resize-y" />
                                                                        ) : (
                                                                            <span className="font-medium text-slate-700 text-xs">{renderWithLatex(card.question || card.front)}</span>
                                                                        )}
                                                                    </td>
                                                                    <td className="px-2 py-2">
                                                                        {editingCardIdx === idx ? (
                                                                            <textarea value={card.answer || card.back || ""} onChange={(e) => { const updated = [...flashcardData]; updated[idx] = { ...updated[idx], answer: e.target.value }; syncFlashcardEditor(updated); }} className="w-full px-2 py-1 text-xs border border-yellow-300 rounded-lg bg-yellow-50 outline-none focus:border-yellow-500 min-h-[60px] resize-y" />
                                                                        ) : (
                                                                            <span className="text-slate-500 text-xs">{renderWithLatex(card.answer || card.back)}</span>
                                                                        )}
                                                                    </td>
                                                                    <td className="px-2 py-2">
                                                                        <div className="flex flex-col gap-1">
                                                                            <button
                                                                                type="button"
                                                                                onClick={() => setEditingCardIdx(editingCardIdx === idx ? null : idx)}
                                                                                className={`p-1 rounded-md transition ${editingCardIdx === idx ? 'bg-yellow-200 text-yellow-700' : 'hover:bg-slate-100 text-slate-400 hover:text-slate-600'}`}
                                                                                title={editingCardIdx === idx ? "เสร็จสิ้น" : "แก้ไข"}
                                                                            >
                                                                            ...
                                                                            <button
                                                                                type="button"
                                                                                onClick={() => {
                                                                                    const remaining = flashcardData.filter((_, i) => i !== idx);
                                                                                    syncFlashcardEditor(remaining);
                                                                                    setSelectedCards(selectedCards.filter(i => i !== idx).map(i => i > idx ? i - 1 : i));
                                                                                    if (editingCardIdx === idx) setEditingCardIdx(null);
                                                                                    showToast(`🗑️ ลบการ์ด #${card.id} แล้ว`);
                                                                                }}
